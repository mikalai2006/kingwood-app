import { defineStore } from "pinia";
import { find, get, remove } from "@/api/task_montaj_worker";
import { IRequestParams } from "@/api/types";
import {
  ITaskMontajWorker,
  ITaskMontajWorkerInput,
} from "@/api/task_montaj_worker/types";
// import sift from 'sift'

export const useTaskMontajWorkerStore = defineStore("taskMontajWorker", {
  state() {
    return {
      _items: [] as ITaskMontajWorker[],
    };
  },
  getters: {
    items(state) {
      return state._items;
    },
  },
  actions: {
    async find(
      params?: IRequestParams<ITaskMontajWorker> | Partial<ITaskMontajWorker>
    ) {
      // const existsItem = this.onExists(params)
      // if (existsItem.index == -1) {
      const data = await find(params || {});
      data.data?.forEach((el) => this.onAddItemToStore(el));
      // }
      return data;
    },
    /**
     * Check exists item to store.
     * @param params
     * @returns
     */
    onExists(id: string): {
      item: ITaskMontajWorker | null;
      index: number;
    } {
      const pageIndex = this._items.findIndex((x) => x.id === id);
      return { item: this._items[pageIndex], index: pageIndex };
    },
    // findInStore(params?: IRequestParams<IUser> | Partial<IUser>): IUser | null {
    //   const item = this._users.filter(sift(params))
    //   console.log('user: findInStore params=', params)
    //   return item[0] || null
    // },
    onAddItemToStore(item: ITaskMontajWorker) {
      const existsItem = this.onExists(item.id);
      if (existsItem.index == -1) {
        this._items.push(item);
      } else {
        this._items[existsItem.index] = Object.assign(
          {},
          this._items[existsItem.index],
          item
        );
      }
    },
    onRemoveItemFromStore(
      id: string | number,
      params?: ITaskMontajWorkerInput
    ): number {
      let itemIndex = -1;
      if (params) {
        // search by params
      } else {
        itemIndex = this._items.findIndex(
          (el: ITaskMontajWorker) => el.id == id
        );
      }

      if (itemIndex !== -1) {
        this._items.splice(itemIndex, 1);
      }

      return itemIndex;
    },
    async onRemove(id: string | undefined) {
      if (!id) {
        return;
      }

      const data = await remove(id, {}).then(() => {
        this.onRemoveItemFromStore(id);
      });

      return data;
    },
    /**
     * Get page by id from server.
     * @param id
     * @param params
     * @returns
     */
    async onGet(
      id: string,
      params?: IRequestParams<ITaskMontajWorker> | Partial<ITaskMontajWorker>
    ) {
      const data = await get(id);
      this.onAddItemToStore(data);
      return data;
    },
    onGetInStore(
      id: string | number,
      params?: ITaskMontajWorker
    ): ITaskMontajWorker | null {
      let item = null;
      if (params) {
        // search by params
      } else {
        item = this._items.find((el: ITaskMontajWorker) => el.id == id);
      }
      return item || null;
    },
  },
});
