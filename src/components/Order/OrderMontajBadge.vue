...
import TaskMontajWorkerStatusTagDot from "../Task/TaskMontajWorkerStatusTagDot.vue";
...
const tasksMontaj = computed(() => {
  return taskMontajStore.items
    .filter((x) => x.objectId === props.objectId)
    .map((z) => {
      const _taskWorkers = taskMontajWorkerStore.items
        .filter(
          (y) =>
            y.taskId === z.id &&
            dayjs(now.value).isBetween(dayjs(y.from), dayjs(y.to), "day", "[]")
        )
        .map((u) => {
...
const allWorkersInOrder = computed(() => {
  return tasksMontaj.value.reduce((ac, el) => {
    ac.push(...el.workers);
    return ac;
  }, [] as ITaskWorker[]);
});
...
onMounted(async () => {
  await taskMontajStore.find({
    objectId: props.objectId,
    $limit: 100,
  });
  await taskMontajWorkerStore.find({
    from: dayjs(new Date()).utc().format(),
    $limit: 100,
  });
});
</script>
<template>
  ...
  >
    ...
    <br /> -->
      ...
      >
        <template #title>
          <div v-for="task in tasksMontaj" :key="task.id" class="mb-4">
            ...
            >
              <TaskMontajWorkerStatusTagDot :task-worker-id="item.id" />
              <div
        ...
        <div class="flex items-center gap-1 space-y-1">
          <TaskMontajWorkerStatusTagDot :task-worker-id="worker.id" />
          <div
            class="text-sm text-nowrap"
            :class="status ? 'text-white' : 'text-black dark:text-g-100'"
          >
